...
```powershell
Install-Module 'icinga-powershell-hyperv';
```
...
```powershell
Use-Icinga;
Install-IcingaFrameworkComponent -Name hyperv -Release;
```
...
```powershell
Use-Icinga;
Install-IcingaFrameworkComponent -Name hyperv -Url 'url to your .zip file';
```

### Manual Installation

For manual installation either download the [latest release .zip](https://github.com/Icinga/icinga-powershell-hyperv/releases) or the [latest master .zip](https://github.com/Icinga/icinga-powershell-hyperv) and extract the content into the correct PowerShell modules folder.
...
```powershell
C:\Program Files\WindowsPowerShell\Modules\icinga-powershell-hyperv-master
```

As our `.psm1` file is named `icinga-powershell-hyperv.psm1` we will have to rename the folder to look like this:

```powershell
C:\Program Files\WindowsPowerShell\Modules\icinga-powershell-hyperv
```

Once this is done, we might require to unblock the file content to be able to load and execute the module

```powershell
Get-ChildItem -Path 'C:\Program Files\WindowsPowerShell\Modules\icinga-powershell-hyperv' -Recurse | Unblock-File;
```
...
```powershell
Import-Module 'icinga-powershell-hyperv';
```

## Using the Hyperv Plugins
